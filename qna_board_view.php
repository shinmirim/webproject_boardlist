...
?>		
	    ...
	    <ul class="buttons">
				...
				<?php
					if($userlevel == 1){
				?>
				<li><button onclick="location.href='qna_board_answer_form.php?num=<?=$num?>&page=<?=$page?>'">답변하기</button></li>
				<?php
					}
				?>
				<?php
					if($userid == $id){
				?>				
				<li><button onclick="location.href='qna_board_delete.php?num=<?=$num?>&page=<?=$page?>'">삭제</button></li>
				<?php
					}

					if($userlevel != 1){
				?>
				<li><button onclick="location.href='qna_board_form.php'">문의하기</button></li>
				<?php
					}
				?>
		</ul>
